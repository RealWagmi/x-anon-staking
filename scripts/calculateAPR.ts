 ...
 */
export async function getPoolAPRMetrics(contract: IXAnonStakingNFTWithAPR, poolId: number): Promise<APRMetrics> {
  // Fetch pool info
  const [, lockDays, rollingActiveStake, , snapshotsCount] = await contract.poolInfo(poolId);

  // Get on-chain APR calculation
  const [aprBasisPoints, confidenceBasisPoints] = await contract.getPoolAPR(poolId, 10);
  ...
  const historicalAPR_7d = calculateHistoricalAPR(recentSnapshots.slice(-7), Number(lockDays));
  const historicalAPR_30d = calculateHistoricalAPR(recentSnapshots, Number(lockDays));
 ...
 */
function calculateHistoricalAPR(snapshots: Array<{ day: bigint; rate: bigint }>, lockDays: number): number {
 ...
 */
export async function compareAllPools(contract: IXAnonStakingNFTWithAPR): Promise<{
  pools: APRMetrics[];
...
function generateRecommendation(pools: APRMetrics[]): string {
  ...
  if (best.dataQuality === 'Poor' || best.dataQuality === 'Fair') {
    recommendations.push(`⚠️ Data quality is ${best.dataQuality}. APR estimates may be unreliable.`);
  }
  ...
  if (shortPool && longPool) {
    ...
    if (riskPremium < 5) {
      recommendations.push(
        `💡 Short pool (${shortPool.projectedAPR.toFixed(2)}% APR) offers similar returns ` +
          `with ${shortPool.lockDays} days lock vs ${longPool.lockDays} days.`,
      );
    } else {
      recommendations.push(`🎯 Long pool offers ${riskPremium.toFixed(2)}% premium APR for extended lock period.`);
    }
  }

  // Best pool recommendation
  recommendations.push(`\n🏆 Best APR: ${best.poolName} with ${best.projectedAPR.toFixed(2)}% projected APR`);

  if (best.confidence < 70) {
    recommendations.push(`\n⚠️ Confidence: ${best.confidence.toFixed(0)}% - Based on limited historical data`);
  }
