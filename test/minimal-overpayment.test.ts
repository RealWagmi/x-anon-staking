...
describe('MINIMAL: Empty Pool Redistribution Test', function () {
  ...
  it('Test: 1 user, 2 topUps with gap', async function () {
    ...
    console.log('Day 5: topUp #1 (1000) → pool2 gets 1000 (100% due to redistribution)');

    // TopUp #2
    await time.increase(3 * DAY);
    await xanonS.connect(owner).topUp(ethers.parseEther('1000'));
    console.log('Day 8: topUp #2 (1000) → pool2 gets 1000 (100% due to redistribution)\n');
    ...
    console.log(`Reason: Pool0 and Pool1 are empty, so ALL rewards go to Pool2 (empty pool redistribution)`);
    console.log('');
    ...
    if (rewards > expected + tolerance) {
      ...
      console.log(`⚠️  UNDERPAYMENT: ${ethers.formatEther(expected - rewards)}`);
