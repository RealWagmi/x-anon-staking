...
describe('xAnonStakingNFT - stake-days weighting', function () {
    ...
    async function deployFixture() {
        ...
        const xanonS = (await XAnonSF.deploy(await anon.getAddress(), await desc.getAddress())) as unknown as XAnonStakingNFT;
    ...
    it('later entrant gets less within the same interval (stake-days)', async function () {
        ...
        expect(aGain).to.be.closeTo(ethers.parseEther('333.333333333333333333'), ethers.parseEther('0.001'));
        expect(bGain).to.be.closeTo(ethers.parseEther('166.666666666666666666'), ethers.parseEther('0.001'));
    ...
    it('same-day entrants share equally for that day', async function () {
        ...
        expect(aGain).to.be.closeTo(ethers.parseEther('250'), ethers.parseEther('0.001'));
        expect(bGain).to.be.closeTo(ethers.parseEther('250'), ethers.parseEther('0.001'));
    });

    it('reverts on topUp with amount below minimum or too frequent or not authorized', async function () {
        ...
        await expect(xanonS.connect(owner).topUp(ethers.parseEther('600'))).to.be.revertedWithCustomError(xanonS, 'TopUpTooFrequent');

        // Same day - alice also can't topUp (any address blocked)
        await expect(xanonS.connect(alice).topUp(ethers.parseEther('600'))).to.be.revertedWithCustomError(xanonS, 'NotAuthorized');
    ...
    it('splits 20/30/50 across pools with equal stake-days', async function () {
        ...
        expect(gainA).to.be.closeTo(ethers.parseEther('500'), ethers.parseEther('0.001'));
        expect(gainB).to.be.closeTo(ethers.parseEther('500'), ethers.parseEther('0.001'));
    });
    ...
    it('caps rewards at expiration (no accrual after lock)', async function () {
        ...
        // Claim
        await xanonS.connect(alice).earnReward(alice.address, 1);
        const gain = (await anon.balanceOf(alice.address)) - (ethers.parseEther('1000000') - ethers.parseEther('100'));
        ...
        expect(gain).to.be.closeTo(ethers.parseEther('180'), ethers.parseEther('0.001'));
    });
    ...
    it('ring buffer expiry shrinks rollingActiveStake after lockDays', async function () {
        ...
        expect(rewardsOldPosition).to.be.closeTo(ethers.parseEther('200'), ethers.parseEther('1'));
    });
    ...
    it('pausable: mint reverts when paused, but earnReward works', async function () {
        ...
        await expect(xanonS.connect(alice).mint(ethers.parseEther('100'), 2)).to.be.revertedWithCustomError(xanonS, 'EnforcedPause');
    ...
    it('emergencyWithdraw: only owner or approved, and only after lock', async function () {
        ...
        await expect(xanonS.connect(bob).emergencyWithdraw(bob.address, 1)).to.be.revertedWithCustomError(xanonS, 'NotAuthorized');

        // Can't withdraw before unlock
        await expect(xanonS.connect(alice).emergencyWithdraw(alice.address, 1)).to.be.revertedWithCustomError(xanonS, 'PositionLocked');
    ...
    it('rescueTokens transfers arbitrary token by owner', async function () {
        ...
        await expect(xanonS.connect(owner).rescueTokens(await anon.getAddress(), owner.address, ethers.parseEther('10'))).to.be.revertedWithCustomError(
            xanonS,
            'CannotRescueAnonToken',
        );
    });
    ...
    it('earnReward then topUp then earnReward: no double rewards, only new interval', async function () {
        ...
        expect(firstClaim).to.be.closeTo(ethers.parseEther('500'), ethers.parseEther('1'));
        ...
        expect(secondClaim).to.be.closeTo(ethers.parseEther('500'), ethers.parseEther('1'));

        // CRITICAL: Total should be ~1000 (500+500), NOT 1500 or more (no double rewards)
        const totalClaimed = firstClaim + secondClaim;
        expect(totalClaimed).to.be.closeTo(ethers.parseEther('1000'), ethers.parseEther('2'));
    ...
    it('transferred NFT allows new owner to claim and burn', async function () {
        ...
        expect(bobRewards).to.be.closeTo(ethers.parseEther('200'), ethers.parseEther('1'));
    ...
    it('reverts on mint with amount exceeding uint96 max (storage packing safety)', async function () {
        ...
        await expect(xanonS.connect(alice).mint(exceedsMax, 0)).to.be.revertedWithCustomError(xanonS, 'AmountExceedsMaximum');
    ...
    it('accumulates rewards correctly across 5+ intervals', async function () {
        ...
        expect(rewards).to.be.closeTo(ethers.parseEther('2500'), ethers.parseEther('10'));
    });
    ...
    it('no accrual when position expires exactly on topUp day after cap', async function () {
        ...
        expect(rewards).to.be.closeTo(ethers.parseEther('200'), ethers.parseEther('1'));
    });
    ...
    it('fair reward distribution - Pool 1 (6 months, 182 days)', async function () {
        ...
        await increaseSeconds(DAY);
        await xanonS.connect(owner).topUp(ethers.parseEther('33333.333333333333333333')); // Pool 1 gets ~10000
        ...
        const totalRewards = aliceRewards + bobRewards + ownerRewards;
        expect(totalRewards).to.be.closeTo(ethers.parseEther('20000'), ethers.parseEther('20'));
    });
    ...
    it('very large gap (1000 days) with partial expirations handles correctly', async function () {
        ...
        expect(bobRewards).to.be.closeTo(ethers.parseEther('5000'), ethers.parseEther('10'));
        expect(ownerRewards).to.be.closeTo(ethers.parseEther('5000'), ethers.parseEther('10'));

        // Total should be close to 15000
        expect(total).to.be.closeTo(ethers.parseEther('15000'), ethers.parseEther('15'));
    });
    ...
    it('pending rewards with very short first interval (1 day) creates valid perDayRate', async function () {
        ...
        expect(actualRewards).to.be.closeTo(ethers.parseEther('10000'), ethers.parseEther('10'));
    });
    ...
    it('intermediate snapshot has correct perDayRate calculation', async function () {
        ...
        expect(rewards).to.be.closeTo(ethers.parseEther('500'), ethers.parseEther('1'));
    });
    ...
    it('CRITICAL: yesterday snapshot math - verify no overpayment from dimension mismatch', async function () {
        ...
        expect(totalPaid).to.be.closeTo(ethers.parseEther('500'), ethers.parseEther('1'));
    ...
    it('constructor reverts with zero address for token', async function () {
        ...
        await expect(XAnonSF.deploy(ethers.ZeroAddress, await desc.getAddress())).to.be.revertedWithCustomError(XAnonSF, 'InvalidTokenAddress');
    });
    ...
    it('constructor reverts with zero address for descriptor', async function () {
        ...
        await expect(XAnonSF.deploy(await anon.getAddress(), ethers.ZeroAddress)).to.be.revertedWithCustomError(XAnonSF, 'InvalidDescriptorAddress');
    });
    ...
    it('rescueTokens() reverts when called by non-owner', async function () {
        ...
        await expect(xanonS.connect(alice).rescueTokens(await anon.getAddress(), alice.address, ethers.parseEther('1'))).to.be.revertedWithCustomError(
            xanonS,
    ...
    it('_earnedDaysInterval: capDay < startDay returns 0 (position expired before interval)', async function () {
        ...
        const balanceBefore = await (await ethers.getContractAt('MockERC20', await xanonS.ANON_TOKEN())).balanceOf(alice.address);
        await xanonS.connect(alice).earnReward(alice.address, 1);
        const balanceAfter = await (await ethers.getContractAt('MockERC20', await xanonS.ANON_TOKEN())).balanceOf(alice.address);
